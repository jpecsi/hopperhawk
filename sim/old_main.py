# ========== IMPORTS ========== #
import network, machine, json, time
...
# API
api = Microdot()
cors = CORS(api, allowed_origins=['*', 'https://hopperhawk.dev', 'http://127.0.0.1', 'http://127.0.0.1:8080', 'http://10.211.55.3', 'http://10.211.55.3:8080'],
...
async def api_status(request):
    try:
        return json.dumps({'alive':True,'pellet_type':config['hopper']['current_pellets'],'pellet_level':(str(current_level)+'%'),'battery_level':(str(battery.get_level())+"%"), 'battery_state':(str(battery.get_status()))})
    except:
...
async def api_calibrate(request,level):
    ...
    if request.method == 'GET':
        ...
        config['hopper'][(str(level)+'_measurement')] = sensor.calibrate(level)
...
async def api_syscontrol(request,action):
    if action == 'reboot':
        ...
        # Wipe out the settings
        config = {
            'wifi': {'status':0,'ssid':"",'password':""},
            'mqtt': {'status':0,'password':"",'client_id':"hopperhawk",'broker_ip':"",'broker_port':1883,'user':""},
            'hopper': {'full_measurement':10,'empty_measurement':180,'current_pellets':"",'poll_frequency':300}
        }
...
async def api_sysconfig(request,setting):
    ...
    if request.method == 'GET':
        if setting == 'wifi':
            return json.dumps({'status':config['wifi']['status'], 'ssid':config['wifi']['ssid']})
        if setting == 'mqtt':
            return json.dumps({'status':config['mqtt']['status'], 'user':config['mqtt']['user'], 'broker_ip':config['mqtt']['broker_ip'], 'broker_port':config['mqtt']['broker_port']})
        if setting == 'hopper':
            return json.dumps(config['hopper'])
    ...
    if request.method == 'POST':
        ...
        if setting == 'wifi':
            ...
            return json.dumps({"configuration":"wifi", "status":200})
        ...
        if setting == 'mqtt':
            ...
            return json.dumps({"configuration":"mqtt", "status":200})

        if setting == 'hopper':
            # Load configs from POST
            config['hopper']['poll_frequency'] = int(request.json['poll_frequency'])
            ...
            return json.dumps({"configuration":"device", "status":200})
# ================================ #
...
def sensor_routine():
    ...
    while True:
        ...
        await asyncio.sleep(int(config['hopper']['poll_frequency']))
